...
final class GradientFillRenderer: PassThroughOutputNode, Renderable {
  ...
  func render(_ inContext: CGContext) {
    ...
    for i in 0..<numberOfColors {
      ...
      if colors.count > ix,
        let color = CGColor(
          colorSpace: colorSpace, components: [colors[ix + 1], colors[ix + 2], colors[ix + 3], 1])
      {
    ...
    guard
      let gradient = CGGradient(
        colorsSpace: colorSpace, colors: gradientColors as CFArray, locations: colorLocations)
    else { return }
    if type == .linear {
      inContext.drawLinearGradient(
        gradient, start: start, end: end,
        options: [.drawsAfterEndLocation, .drawsBeforeStartLocation])
    } else {
      inContext.drawRadialGradient(
        gradient, startCenter: start, startRadius: 0, endCenter: start,
        endRadius: start.distanceTo(end),
        options: [.drawsAfterEndLocation, .drawsBeforeStartLocation])
    }
  }
