...
import Foundation
  ...
  final public class AnimatedSwitch: AnimatedControl {
    ...
    func updateOnState(isOn: Bool, animated: Bool, shouldFireHaptics: Bool) {
      ...
      animationView.play(
        fromProgress: startProgress, toProgress: endProgress, loopMode: LottieLoopMode.playOnce
      ) { (finished) in
        if finished == true {
    ...
    private func updateAccessibilityLabel() {
      accessibilityValue =
        _isOn ? NSLocalizedString("On", comment: "On") : NSLocalizedString("Off", comment: "Off")
    }
