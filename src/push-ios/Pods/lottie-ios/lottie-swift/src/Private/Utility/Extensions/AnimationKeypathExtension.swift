...
extension KeypathSearchable {
  ...
  func layer(for keyPath: AnimationKeypath) -> CALayer? {
    if keyPath.nextKeypath == nil, let layerKey = keyPath.currentKey,
      layerKey.equalsKeypath(keypathName)
    {
      /// We found our layer!
      return keypathLayer
    }
  ...
  func equalsKeypath(_ keyname: String) -> Bool {
    ...
    if let index = self.firstIndex(of: "*") {
      ...
      if prefix.count > 0 {
        ...
        let testPrefix = keyname.prefix(
          upTo: keyname.index(keyname.startIndex, offsetBy: prefix.count))
