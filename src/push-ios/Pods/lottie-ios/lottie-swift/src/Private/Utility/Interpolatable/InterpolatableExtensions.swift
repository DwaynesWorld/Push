...
extension Vector1D: Interpolatable {
  func interpolateTo(
    _ to: Vector1D, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> Vector1D {
...
extension Vector2D: Interpolatable {
  func interpolateTo(
    _ to: Vector2D, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> Vector2D {
    return pointValue.interpolateTo(
      to.pointValue, amount: CGFloat(amount), spatialOutTangent: spatialOutTangent,
...
extension Vector3D: Interpolatable {
  func interpolateTo(
    _ to: Vector3D, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> Vector3D {
    if spatialInTangent != nil || spatialOutTangent != nil {
      ...
      let point = pointValue.interpolateTo(
        to.pointValue, amount: amount, spatialOutTangent: spatialOutTangent,
        spatialInTangent: spatialInTangent)
      return Vector3D(
        x: point.x,
        y: point.y,
...
extension Color: Interpolatable {
  ...
  func interpolateTo(
    _ to: Color, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> Color {
    return Color(
...
extension CurveVertex: Interpolatable {
  func interpolateTo(
    _ to: CurveVertex, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> CurveVertex {
    return CurveVertex(
...
extension BezierPath: Interpolatable {
  func interpolateTo(
    _ to: BezierPath, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> BezierPath {
    ...
    for i in 0..<min(elements.count, to.elements.count) {
      ...
      newPath.addVertex(
        fromVertex.interpolateTo(
          toVertex, amount: amount, spatialOutTangent: spatialOutTangent,
          spatialInTangent: spatialInTangent))
    }
    return newPath
  }
...
extension TextDocument: Interpolatable {
  ...
  func interpolateTo(
    _ to: TextDocument, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> TextDocument {
    if amount == 1 {
      return to
    }
...
extension Array: Interpolatable where Element == Double {
  func interpolateTo(
    _ to: [Element], amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> [Element] {
