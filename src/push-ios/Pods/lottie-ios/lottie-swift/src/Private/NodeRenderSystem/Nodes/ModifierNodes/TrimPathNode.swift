...
final class TrimPathNode: AnimatorNode {
  ...
  func rebuildOutputs(frame: CGFloat) {
      ...
      for pathContainer in upstreamPaths {
        ...
        for path in pathObjects {
          ...
          pathContainer.appendPath(
            path.trim(
              fromPosition: start, toPosition: end, offset: offset, trimSimultaneously: false),
            updateFrame: frame)
        }
      }
      return
    ...
    for pathContainer in upstreamPaths {
      ...
      if !startLength.isInRange(pathStart, pathEnd) && endLength.isInRange(pathStart, pathEnd) {
        // pathStart|=======E----------------------|pathEnd
        ...
        for path in subpaths {
          ...
          if pathCutLength < subpathEnd {
            ...
            let newPath = path.trim(
              fromPosition: 0, toPosition: cutLength / path.length, offset: 0,
        ...
        && startLength.isInRange(pathStart, pathEnd)
      {
        ...
        for path in subpaths {
          ...
          if subpathStart < pathCutLength, pathCutLength < subpathEnd {
            ...
            let cutLength = pathCutLength - subpathStart
            let newPath = path.trim(
              fromPosition: cutLength / path.length, toPosition: 1, offset: 0,
              trimSimultaneously: false)
            pathContainer.appendPath(newPath, updateFrame: frame)
      ...
      } else if endLength.isInRange(pathStart, pathEnd) && startLength.isInRange(pathStart, pathEnd)
      {
        ...
        for path in subpaths {
            ...
            && endCutLength.isInRange(subpathStart, subpathEnd)
          {
            ...
            let newPath = path.trim(
              fromPosition: cutFromLength / path.length, toPosition: cutToLength / path.length,
              offset: 0, trimSimultaneously: false)
            pathContainer.appendPath(newPath, updateFrame: frame)
            break
          }
