...
extension AnimatorNode {
  ...
  @discardableResult func updateOutputs(_ frame: CGFloat, forceOutputUpdate: Bool) -> Bool {
    ...
    hasUpstreamUpdates =
      (parentNode?.updateOutputs(frame, forceOutputUpdate: forceUpstreamUpdates) ?? false
        || hasUpstreamUpdates)
  ...
  @discardableResult func updateContents(_ frame: CGFloat, forceLocalUpdate: Bool) -> Bool {
    ...
    if forceLocalUpdate == false && lastUpdateFrame != nil && lastUpdateFrame! == frame {
      /// This node has already updated for this frame. Go ahead and return the results.
      return localUpdatesPermeateDownstream()
        ? hasUpstreamUpdates || hasLocalUpdates : hasUpstreamUpdates
    }

    /// Are there local updates? If so mark the node.
    hasLocalUpdates =
      forceLocalUpdate ? forceLocalUpdate : propertyMap.needsLocalUpdate(frame: frame)

    /// Were there upstream updates? If so mark the node
    hasUpstreamUpdates =
      parentNode?.updateContents(frame, forceLocalUpdate: forceLocalUpdate) ?? false
    ...
    hasUpstreamUpdates =
      performAdditionalLocalUpdates(frame: frame, forceLocalUpdate: forceLocalUpdate)
      || hasUpstreamUpdates
