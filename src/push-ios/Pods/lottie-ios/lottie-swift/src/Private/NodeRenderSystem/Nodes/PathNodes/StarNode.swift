...
final class StarNodeProperties: NodePropertyMap, KeypathSearchable {
  ...
  init(star: Star) {
    ...
    self.outerRadius = NodeProperty(
      provider: KeyframeInterpolator(keyframes: star.outerRadius.keyframes))
    self.outerRoundedness = NodeProperty(
      provider: KeyframeInterpolator(keyframes: star.outerRoundness.keyframes))
    if let innerRadiusKeyframes = star.innerRadius?.keyframes {
      self.innerRadius = NodeProperty(
        provider: KeyframeInterpolator(keyframes: innerRadiusKeyframes))
    } else {
    ...
    if let innderRoundedness = star.innerRoundness?.keyframes {
      self.innerRoundedness = NodeProperty(
        provider: KeyframeInterpolator(keyframes: innderRoundedness))
    } else {
...
final class StarNode: AnimatorNode, PathNode {
  ...
  func rebuildOutputs(frame: CGFloat) {
    ...
    vertices.append(
      CurveVertex(point: point + position, inTangentRelative: .zero, outTangentRelative: .zero))
    ...
    for i in 0..<numPoints {
      ...
      if innerRoundedness == 0 && outerRoundedness == 0 {
        ...
        vertices[vertices.endIndex - 1] = CurveVertex(
          previousVertex.inTangent, previousVertex.point, previousVertex.point - cp1 + position)
        vertices.append(
          CurveVertex(point: point + position, inTangentRelative: cp2, outTangentRelative: .zero))
      }
      currentAngle += dTheta
