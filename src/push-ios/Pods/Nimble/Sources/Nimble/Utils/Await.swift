...
internal class AwaitPromiseBuilder<T> {
  ...
  func wait(_ fnName: String = #function, file: FileString = #file, line: UInt = #line)
    -> AwaitResult<T>
  {
...
internal class Awaiter {
  ...
  func poll<T>(_ pollInterval: TimeInterval, closure: @escaping () throws -> T?)
    -> AwaitPromiseBuilder<T>
  {
    let promise = AwaitPromise<T>()
    let timeoutSource = createTimerSource(timeoutQueue)
    ...
    let trigger = AwaitTrigger(timeoutSource: timeoutSource, actionSource: asyncSource) {
      let interval = DispatchTimeInterval.nanoseconds(
        Int(pollInterval * TimeInterval(NSEC_PER_SEC)))
...
internal func pollBlock(
  ...
  let result = awaiter.poll(pollInterval) { () throws -> Bool? in
    ...
    return nil
  }.timeout(timeoutInterval, forcefullyAbortTimeout: timeoutInterval / 2.0).wait(
    fnName, file: file, line: line)
