...
final public class AnimationView: LottieView {
  ...
  public var realtimeAnimationFrame: AnimationFrameTime {
    return isAnimationPlaying
      ? animationLayer?.presentation()?.currentFrame ?? currentFrame : currentFrame
  }
  ...
  */
  ...
  public init(
    animation: Animation?, imageProvider: AnimationImageProvider? = nil,
  ...
  override func layoutAnimation() {
    ...
    if let viewportFrame = self.viewportFrame {
      ...
      xform = CATransform3DTranslate(
        CATransform3DMakeScale(scale, scale, 1), viewportOffset.x, viewportOffset.y, 0)
    ...
    if let key = viewLayer?.animationKeys()?.first,
      let animation = viewLayer?.animation(forKey: key)
    {
      ...
      let xformAnimation =
        animation.copy() as? CABasicAnimation ?? CABasicAnimation(keyPath: "transform")
  ...
  fileprivate func makeAnimationLayer() {
    ...
    let animationLayer = AnimationContainer(
      animation: animation, imageProvider: imageProvider, textProvider: textProvider)
  ...
  fileprivate func addNewAnimationForContext(_ animationContext: AnimationContext) {
    ...
    let playingForward: Bool =
      ((animationSpeed > 0 && playFrom < playTo) || (animationSpeed < 0 && playTo < playFrom))
