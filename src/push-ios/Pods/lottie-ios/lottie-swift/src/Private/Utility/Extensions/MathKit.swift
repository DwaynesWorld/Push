...
extension CGFloat: Interpolatable {
  ...
  func interpolateTo(
    _ to: CGFloat, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> CGFloat {
...
extension Double: Interpolatable {
  ...
  func interpolateTo(
    _ to: Double, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> Double {
    return interpolateTo(to, amount: amount)
  }
...
extension CGPoint: Interpolatable {
  ...
  func interpolate(
    _ to: CGPoint, outTangent: CGPoint, inTangent: CGPoint, amount: CGFloat, maxIterations: Int = 3,
    samples: Int = 20, accuracy: CGFloat = 1
  ) -> CGPoint {
    if amount == 0 {
      return self
    }
    ...
    while previousAmount < 1 {
      ...
      let newPoint = self.pointOnPath(
        to, outTangent: outTangent, inTangent: inTangent, amount: previousAmount)
    ...
    while foundPoint == false {
      ...
      let proposedPointAmount = ((accurateDistance - point.distance) / pointDiff).remap(
        fromLow: 0, fromHigh: 1, toLow: pointAmount, toHigh: nextPointAmount)

      let newPoint = self.pointOnPath(
        to, outTangent: outTangent, inTangent: inTangent, amount: proposedPointAmount)
      ...
      if accurateDistance - newDistance <= accuracy || newDistance - accurateDistance <= accuracy {
        foundPoint = true
      }
  ...
  func pointOnPath(_ to: CGPoint, outTangent: CGPoint, inTangent: CGPoint, amount: CGFloat)
    -> CGPoint
  {
  ...
  func interpolateTo(
    _ to: CGPoint, amount: CGFloat, spatialOutTangent: CGPoint?, spatialInTangent: CGPoint?
  ) -> CGPoint {
