...
struct BezierPath {
  ...
  mutating func addCurve(toPoint: CGPoint, outTangent: CGPoint, inTangent: CGPoint) {
    ...
    updateVertex(
      ...
      atIndex: elements.endIndex - 1, remeasure: false)
    addVertex(newVertex)
  }
  ...
  func trim(fromLength: CGFloat, toLength: CGFloat, offsetLength: CGFloat) -> [BezierPath] {
    ...
    if start == 0 && end == length || start == end || start == length && end == 0 {
  ...
  fileprivate func trimPathAtLengths(positions: [(start: CGFloat, end: CGFloat)]) -> [BezierPath] {
    ...
    while !finishedTrimming {
      ...
      if runningLength < trim.start, trim.start < newLength, currentPath.elements.count == 0 {
        ...
        let trimResults = element.splitElementAtPosition(
          fromElement: previousElement, atLength: trimLength)
      ...
      if runningLength < trim.end, trim.end < newLength {
        ...
        currentPath.updateVertex(
          trimResults.leftSpan.start.vertex, atIndex: currentPath.elements.count - 1,
          remeasure: false)
...
extension BezierPath: Codable {
  ...
  init(from decoder: Decoder) throws {
    ...
    guard vertexContainer.count == inPointsContainer.count,
      inPointsContainer.count == outPointsContainer.count
    else {
    ...
    for element in elements {
      if let previous = previousElement {
        if previous.vertex.outTangentRelative.isZero && element.vertex.inTangentRelative.isZero {
          ...
          cgPath.addCurve(
            to: element.vertex.point, control1: previous.vertex.outTangent,
            control2: element.vertex.inTangent)
        }
      } else {
