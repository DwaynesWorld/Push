...
public func postNotifications(
  ...
  return Predicate { actualExpression in
    ...
    assert(
      pthread_equal(mainThread, pthread_self()) != 0,
      "Only expecting closure to be evaluated on main thread.")
    if !once {
      once = true
...
public func postNotifications<T>(
  ...
  return Predicate { actualExpression in
    ...
    let match = try notificationsMatcher.matches(
      collectorNotificationsExpression, failureMessage: failureMessage)
    if collector.observedNotifications.isEmpty {
