...
struct CurveVertex {
  ...
  func translated(_ translation: CGPoint) -> CurveVertex {
    return CurveVertex(
      point: point + translation, inTangent: inTangent + translation,
      outTangent: outTangent + translation)
  }
  ...
  func splitCurve(toVertex: CurveVertex, position: CGFloat) -> (
    ...
    if position <= 0 {
      return (
        start: CurveVertex(
          point: point, inTangentRelative: inTangentRelative, outTangentRelative: .zero),
        trimPoint: CurveVertex(
          point: point, inTangentRelative: .zero, outTangentRelative: outTangentRelative),
        end: toVertex
      )
    }
    ...
    if position >= 1 {
      return (
        start: self,
        trimPoint: CurveVertex(
          point: toVertex.point, inTangentRelative: toVertex.inTangentRelative,
          outTangentRelative: .zero),
        end: CurveVertex(
          point: toVertex.point, inTangentRelative: .zero,
    ...
    if outTangentRelative.isZero && toVertex.inTangentRelative.isZero {
      ...
      return (
        start: self,
        trimPoint: CurveVertex(
          point: trimPoint, inTangentRelative: .zero, outTangentRelative: .zero),
        end: toVertex
      )
    }
  ...
  */
  func trimCurve(
    toVertex: CurveVertex, atLength: CGFloat, curveLength: CGFloat, maxSamples: Int,
    accuracy: CGFloat = 1
  ) -> (start: CurveVertex, trimPoint: CurveVertex, end: CurveVertex) {
    ...
    for _ in 1...maxSamples {
      ...
      let diffPosition = max(
        min(((currentPosition / length) * lengthDiff), currentPosition * 0.5),
        currentPosition * -0.5)
  ...
  func distanceTo(_ toVertex: CurveVertex, sampleCount: Int = 25) -> CGFloat {
    ...
    for i in 0..<sampleCount {
      let pointOnCurve = splitCurve(toVertex: toVertex, position: CGFloat(i) / CGFloat(sampleCount))
        .trimPoint
