...
// This matcher requires the Objective-C, and being built by Xcode rather than the Swift Package Manager
  ...
  internal func setFailureMessageForException(
    ...
    if let exception = exception {
      ...
      failureMessage.actualValue =
        "\(String(describing: type(of: exception))) { name=\(exception.name), reason='\(stringify(exception.reason))', userInfo=\(stringify(exception.userInfo)) }"
    } else {
  ...
  public class NMBObjCRaiseExceptionMatcher: NSObject, NMBMatcher {
    ...
    internal init(
      name: String?, reason: String?, userInfo: NSDictionary?, block: ((NSException) -> Void)?
    ) {
      _name = name
      _reason = reason
      _userInfo = userInfo
    ...
    @objc public var satisfyingBlock:
      (_ block: ((NSException) -> Void)?) -> NMBObjCRaiseExceptionMatcher
    {
      return { block in
        return NMBObjCRaiseExceptionMatcher(
          name: self._name,
