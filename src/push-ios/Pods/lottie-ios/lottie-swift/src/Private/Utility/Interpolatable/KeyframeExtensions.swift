...
extension Keyframe {
  ...
  func interpolatedProgress(_ to: Keyframe, keyTime: CGFloat) -> CGFloat {
    ...
    var progress: CGFloat = keyTime.remap(
      fromLow: startTime, fromHigh: endTime, toLow: 0, toHigh: 1)
    if !outTanPoint.isZero || !inTanPoint.equalTo(CGPoint(x: 1, y: 1)) {
      /// Cubic interpolation
      progress = progress.cubicBezierInterpolate(
        .zero, outTanPoint, inTanPoint, CGPoint(x: 1, y: 1))
    }
    return progress
  }
  ...
  func interpolate(_ to: Keyframe, progress: CGFloat) -> T {
    return value.interpolateTo(
      to.value, amount: progress, spatialOutTangent: spatialOutTangent?.pointValue,
