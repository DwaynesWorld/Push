...
//
  ...
  extension request_mach_exception_raise_t {
    mutating func withMsgHeaderPointer<R>(in block: (UnsafeMutablePointer<mach_msg_header_t>) -> R)
      -> R
    {
  ...
  private struct MachContext {
    ...
    static func internalMutablePointers<R>(
      _ m: UnsafeMutablePointer<execTypesCountTuple<exception_mask_t>>,
      _ c: UnsafeMutablePointer<mach_msg_type_number_t>,
      _ p: UnsafeMutablePointer<execTypesCountTuple<mach_port_t>>,
      _ b: UnsafeMutablePointer<execTypesCountTuple<exception_behavior_t>>,
      _ f: UnsafeMutablePointer<execTypesCountTuple<thread_state_flavor_t>>,
      _ block: (
        UnsafeMutablePointer<exception_mask_t>, UnsafeMutablePointer<mach_msg_type_number_t>,
        UnsafeMutablePointer<mach_port_t>, UnsafeMutablePointer<exception_behavior_t>,
        UnsafeMutablePointer<thread_state_flavor_t>
      ) -> R
    ) -> R {
        ...
        return c.withMemoryRebound(to: mach_msg_type_number_t.self, capacity: 1) { countPtr in
          return p.withMemoryRebound(to: mach_port_t.self, capacity: 1) { portsPtr in
            return b.withMemoryRebound(to: exception_behavior_t.self, capacity: 1) { behaviorsPtr in
              return f.withMemoryRebound(to: thread_state_flavor_t.self, capacity: 1) {
                flavorsPtr in
    ...
    mutating func withUnsafeMutablePointers<R>(
      ...
      return MachContext.internalMutablePointers(
        &masks, &count, &ports, &behaviors, &flavors, block)
    }
  }
  ...
  private func machMessageHandler(_ arg: UnsafeMutableRawPointer) -> UnsafeMutableRawPointer? {
    ...
    repeat {
      do {
        ...
        try kernCheck {
          request.withMsgHeaderPointer { requestPtr in
            mach_msg(
              requestPtr, MACH_RCV_MSG | MACH_RCV_INTERRUPT, 0, requestSize,
        ...
        if !handledfirstException {
          ...
          guard
            request.withMsgHeaderPointer(in: { requestPtr in
              reply.withMsgHeaderPointer { replyPtr in
                mach_exc_server(requestPtr, replyPtr)
              }
            }) != 0
          else { throw MachExcServer.code(reply.RetCode) }
        ...
        try kernCheck {
          reply.withMsgHeaderPointer { replyPtr in
            mach_msg(
              replyPtr, MACH_SEND_MSG, replySize, 0, UInt32(MACH_PORT_NULL), 0,
              UInt32(MACH_PORT_NULL))
          }
        }
      } catch let error as NSError
        where
        (error.domain == NSMachErrorDomain
        && (error.code == Int(MACH_RCV_PORT_CHANGED) || error.code == Int(MACH_RCV_INVALID_NAME)))
      {
    ...
    do {
      ...
      try kernCheck {
        ...
        mach_port_insert_right(
          mach_task_self_, context.currentExceptionPort, context.currentExceptionPort,
          MACH_MSG_TYPE_MAKE_SEND)
      }

      let currentExceptionPtr = context.currentExceptionPort
      try kernCheck {
        context.withUnsafeMutablePointers {
          masksPtr, countPtr, portsPtr, behaviorsPtr, flavorsPtr in
          // 3. Apply the mach port as the handler for this thread
          thread_swap_exception_ports(
            mach_thread_self(), EXC_MASK_BAD_INSTRUCTION, currentExceptionPtr,
            Int32(bitPattern: UInt32(EXCEPTION_STATE) | MACH_EXCEPTION_CODES), x86_THREAD_STATE64,
            masksPtr, countPtr, portsPtr, behaviorsPtr, flavorsPtr)
        }
      }
      ...
      defer {
        context.withUnsafeMutablePointers {
          ...
          _ = thread_swap_exception_ports(
            mach_thread_self(), EXC_MASK_BAD_INSTRUCTION, 0, EXCEPTION_DEFAULT, THREAD_STATE_NONE,
            masksPtr, countPtr, portsPtr, behaviorsPtr, flavorsPtr)
        }
      }
