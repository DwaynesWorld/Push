...
extension AnimationView {
  ...
  public convenience init(
    ...
    let animation = Animation.named(
      name, bundle: bundle, subdirectory: nil, animationCache: animationCache)
  ...
  public convenience init(
    ...
    let provider =
      ...
      ?? FilepathImageProvider(
        filepath: URL(fileURLWithPath: filePath).deletingLastPathComponent().path)
    self.init(animation: animation, imageProvider: provider)
  }
  ...
  public convenience init(
    ...
    if let animationCache = animationCache,
      let animation = animationCache.animation(forKey: url.absoluteString)
    {
