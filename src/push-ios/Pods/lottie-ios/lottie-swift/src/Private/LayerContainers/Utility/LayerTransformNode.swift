...
final class LayerTransformProperties: NodePropertyMap, KeypathSearchable {
  ...
  init(transform: Transform) {
    ...
    self.anchor = NodeProperty(
      provider: KeyframeInterpolator(keyframes: transform.anchorPoint.keyframes))
    ...
    {
      let xPosition: NodeProperty<Vector1D> = NodeProperty(
        provider: KeyframeInterpolator(keyframes: positionKeyframesX))
      let yPosition: NodeProperty<Vector1D> = NodeProperty(
        provider: KeyframeInterpolator(keyframes: positionKeyframesY))
      ...
      let position: NodeProperty<Vector3D> = NodeProperty(
        provider: KeyframeInterpolator(keyframes: positionKeyframes))
